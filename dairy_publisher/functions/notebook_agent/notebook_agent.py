...
INDEX_ID = os.getenv("INDEX_ID", "your-index-id")
INDEX_ENDPOINT_ID = os.getenv("INDEX_ENDPOINT_ID", "your-index-endpoint-id")
...
def analyze_period_and_themes(
    ...
    """
    try:
        logger.info(f"Analyzing period for child {child_id}: {start_date} to {end_date}")
        ...
        themes = [
            {
                "id": "interest",
                "title": "今週の興味",
                "search_queries": ["興味", "夢中", "好き", "楽しい", "お気に入り", "遊び", "おもちゃ"],
                "prompt_hint": "子供が今週特に興味を持ったことや夢中になったことについて"
            },
            {
                "id": "place",
                "title": "行った！場所",
                "search_queries": ["行った", "お出かけ", "公園", "散歩", "訪問", "外出", "おでかけ"],
            ...
            },
            {
                "id": "best_shot",
                "title": "今週のベストショット",
                "search_queries": ["笑顔", "かわいい", "素敵", "最高", "楽しそう", "嬉しそう"],
                "prompt_hint": "今週の最も印象的な瞬間や表情"
            },
            {
                "id": "achievement",
                "title": "できるようになったこと",
                "search_queries": ["できた", "成長", "上手", "覚えた", "言えた", "できるように"],
...
def collect_episodes_by_theme(
    ...
    try:
        ...
        query = episodes_ref.where('child_id', '==', child_id)\
        ...
        for doc in query.stream():
            ...
            tags = [tag.lower() for tag in episode_data.get('tags', [])]
            
            # いずれかの検索クエリがコンテンツまたはタグに含まれているか
            matches = False
            for query in theme_info['search_queries']:
            ...
            if matches:
                episodes.append({
                    ...
                    'image_urls': episode_data.get('image_urls', []),
...
def generate_topic_content(
    ...
    """
    try:
        theme = theme_episodes['theme']
        episodes = theme_episodes['episodes']
        
        logger.info(f"Generating content for theme: {theme['title']} with {len(episodes)} episodes")
        ...
        if topic_layout == "large_photo" and episodes:
            # 最初のエピソードからキーワードを抽出
            subtitle = episodes[0]['tags'][0] if episodes[0].get('tags') else theme['title']
...
def validate_and_save_notebook(
    ...
    try:
        ...
        if valid_topics == 0:
            ...
            status_message = f"{len(missing_topics)}個のトピックでコンテンツが不足しています"
        else:
            status = "success"
        ...
        notebook_ref = db.collection('children').document(child_id)\
                        .collection('notebooks').document(notebook_data['notebook_id'])
...
root_agent = Agent(
    ...
    注意事項：
    - エピソードが見つからないテーマでも、デフォルトコンテンツを生成すること
...
def generate_notebook_for_child(
    child_id: str,
    start_date: str,
    end_date: str,
    child_info: Optional[Dict[str, Any]] = None
    ...
    try:
        ...
        result = root_agent.run(
            f"子供ID: {child_id}の{start_date}から{end_date}までのノートブックを生成してください。"
            f"子供の情報: {json.dumps(child_info, ensure_ascii=False)}"
        )
        ...
        logger.error(f"Error in generate_notebook_for_child: {str(e)}")
        return {
            "status": "error",
            "error_message": str(e)
        }
...
if __name__ == "__main__":
    # テスト実行
    result = generate_notebook_for_child(
        child_id="taro_2020",
